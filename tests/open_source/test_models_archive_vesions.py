...
def _run_test(mlflow_context, stage, num_stages, archive_existing_versions=False):
    ...
    model_src, model_dst = _run_export_import(
        mlflow_context, stages, archive_existing_versions
    )
...
def _run_export_import(mlflow_context, stages, archive_existing_versions=False):
    ...
    for stage in stages:
        oss_utils_test.create_version(
            mlflow_context.client_src, model_name_src, stage, archive_existing_versions
        )
...
def _compare_models_with_versions(mlflow_context, model_src, model_dst):
    ...
    compare_models(
        model_src, model_dst, mlflow_context.client_src != mlflow_context.client_dst
    )
