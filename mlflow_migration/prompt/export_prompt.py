...
def export_prompt(
    ...
    try:
        ...
        mlflow_attr = {"prompt": prompt_info}

        # Write prompt export file
        io_utils.write_export_file(output_dir, "prompt.json", __file__, mlflow_attr)

        _logger.info(
            f"Successfully exported prompt: {prompt_name} version {prompt_version}"
        )
        return prompt

    except RestException as e:
        _logger.error(
            f"Prompt export failed: {{'prompt_name': '{prompt_name}', 'version': '{prompt_version}', 'RestException': {e.json}}}"
        )
        return None
    except Exception as e:
        _logger.error(
            f"Prompt export failed: {{'prompt_name': '{prompt_name}', 'version': '{prompt_version}', 'Exception': {str(e)}}}"
        )
        return None


def _get_prompt_safe(prompt_name: str, prompt_version: str) -> object:
    """
    ...
    try:
        ...
        _logger.debug(
            f"mlflow.genai.load_prompt failed for prompt '{prompt_name}' version '{prompt_version}': {e}"
        )
    ...
    try:
        ...
        _logger.debug(
            f"MlflowClient.get_prompt_version failed for prompt '{prompt_name}' version '{prompt_version}': {e}"
        )
    ...
    try:
        ...
        _logger.debug(
            f"mlflow.load_prompt failed for prompt '{prompt_name}' version '{prompt_version}': {e}"
        )

    raise Exception(
        f"No compatible prompt loading API found in MLflow {mlflow.__version__}. Ensure prompt registry is supported."
    )


@click.command()
@click.option(
    "--prompt-name", help="Name of the prompt to export.", type=str, required=True
)
@click.option(
    "--prompt-version", help="Version of the prompt to export.", type=str, required=True
)
@opt_output_dir
def main(prompt_name, prompt_version, output_dir):
    ...
    export_prompt(
        prompt_name=prompt_name, prompt_version=prompt_version, output_dir=output_dir
    )
