...
def import_prompts(
    ...
    try:
        ...
        # Import prompts
        if use_threads:
            results = _import_prompts_threaded(
                prompt_dirs, mlflow_client, delete_prompt
            )
        else:
            results = _import_prompts_sequential(
                prompt_dirs, mlflow_client, delete_prompt
            )
    ...
    for item in sorted(os.listdir(input_dir)):
        ...
        if os.path.isdir(item_path):
            ...
            if os.path.exists(prompt_file):
                prompt_dirs.append({"name": item, "path": item_path})
...
def _import_prompts_threaded(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        results: list[tuple[str, int] | None] = list(
            executor.map(import_single, prompt_dirs)
        )
...
@click.option(
    "--use-threads", help="Use multithreading for import.", is_flag=True, default=False
)
def main(input_dir, delete_prompt, use_threads):
    ...
    result = import_prompts(
        input_dir=input_dir, delete_prompt=delete_prompt, use_threads=use_threads
    )
