...
from mlflow_migration.common.version_utils import (
    has_prompt_support,
    log_version_info,
    get_mlflow_version,
    get_version_info,
)
from mlflow_migration.common.prompt_utils import delete_prompt as delete_prompt_util
...
def _check_import_compatibility(prompt_data):
    ...
    try:
        ...
        if not target_info["supports_prompts"]:
            raise Exception(
                f"Target MLflow {target_version} does not support prompt registry (requires 2.21.0+)"
            )
        ...
        if source_ver >= version.parse("3.0.0") and target_ver < version.parse("3.0.0"):
            _logger.warning(
                "Importing from MLflow 3.x to 2.x - some metadata may be lost"
            )
...
def import_prompt(
    ...
    try:
        ...
        _logger.debug(
            f"Creating prompt '{final_prompt_name}' with template: {prompt_info.get('template', '')[:50]}..."
        )
        ...
        if imported_prompt is None:
            _logger.error(
                f"Failed to create prompt '{final_prompt_name}' - _create_prompt_safe returned None"
            )
            return None
        ...
        return (
            final_prompt_name,
            imported_prompt.version if hasattr(imported_prompt, "version") else "1",
        )
...
def _create_prompt_safe(
    ...
    raise Exception(
        f"No compatible prompt creation API found in MLflow {mlflow.__version__}. Ensure prompt registry is supported."
    )
...
def main(input_dir, prompt_name, delete_prompt):
    ...
    import_prompt(
        input_dir=input_dir, prompt_name=prompt_name, delete_prompt=delete_prompt
    )
