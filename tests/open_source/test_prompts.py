...
def _create_test_prompt(
    ...
    try:
        ...
        if hasattr(mlflow.genai, "register_prompt"):
            return mlflow.genai.register_prompt(
                name=name, template=template, tags=tags or {}
            )
    except Exception:
    ...
    if hasattr(mlflow, "register_prompt"):
        return mlflow.register_prompt(name=name, template=template, tags=tags or {})
...
def test_bulk_export_import_prompts(mlflow_context):
    ...
    for name, template in prompts_data:
        _create_test_prompt(mlflow_context.client_src, name=name, template=template)
    ...
    import_result = import_prompts(
        input_dir=output_dir, use_threads=False, mlflow_client=mlflow_context.client_dst
    )
